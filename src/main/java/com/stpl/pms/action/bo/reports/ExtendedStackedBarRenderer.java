...
public class ExtendedStackedBarRenderer extends StackedBarRenderer {
	...
	public ExtendedStackedBarRenderer() {
		...
		totalLabelFont = new Font("Arial", Font.PLAIN, 9);
	...
	public void drawItem(Graphics2D graphics2d,
		...
		double d1 = categoryaxis.getCategoryMiddle(j, getColumnCount(),
				rectangle2d, categoryplot.getDomainAxisEdge())
		...
		org.jfree.ui.RectangleEdge rectangleedge = categoryplot
				.getRangeAxisEdge();
		double d4;
		double d6;
		if (d > 0.0D) {
		...
		if (plotorientation == PlotOrientation.HORIZONTAL)
			double1 = new java.awt.geom.Rectangle2D.Double(d7, d1, d8,
					categoryitemrendererstate.getBarWidth());
		else
			double1 = new java.awt.geom.Rectangle2D.Double(d1, d7,
					categoryitemrendererstate.getBarWidth(), d8);
		...
		org.jfree.chart.labels.CategoryItemLabelGenerator categoryitemlabelgenerator = getItemLabelGenerator(
				i, j);
		if (categoryitemlabelgenerator != null && isItemLabelVisible(i, j))
			drawItemLabel(graphics2d, categorydataset, i, j, categoryplot,
					categoryitemlabelgenerator, double1, d < 0.0D);
		if (d > 0.0D) {
			if (showPositiveTotal && isLastPositiveItem(categorydataset, i, j)) {
				graphics2d.setPaint(Color.black);
				graphics2d.setFont(totalLabelFont);
				double d9 = calculateSumOfPositiveValuesForCategory(
						categorydataset, j);
				...
				TextUtilities.drawRotatedString(totalFormatter.format(d9),
						graphics2d, f, f2, textanchor, 0.0D, TextAnchor.CENTER);
			}
		} else if (showNegativeTotal
				&& isLastNegativeItem(categorydataset, i, j)) {
			graphics2d.setPaint(Color.black);
			graphics2d.setFont(totalLabelFont);
			double d10 = calculateSumOfNegativeValuesForCategory(
					categorydataset, j);
			...
			TextUtilities.drawRotatedString(totalFormatter.format(d10),
					graphics2d, f1, f3, textanchor1, 0.0D, TextAnchor.CENTER);
		}
		if (categoryitemrendererstate.getInfo() != null) {
			EntityCollection entitycollection = categoryitemrendererstate
					.getEntityCollection();
			if (entitycollection != null) {
				String s = null;
				CategoryToolTipGenerator categorytooltipgenerator = getToolTipGenerator(
						i, j);
				if (categorytooltipgenerator != null)
					s = categorytooltipgenerator.generateToolTip(
							categorydataset, i, j);
				String s1 = null;
				if (getItemURLGenerator(i, j) != null)
					s1 = getItemURLGenerator(i, j).generateURL(categorydataset,
							i, j);
				CategoryItemEntity categoryitementity = new CategoryItemEntity(
						double1, s, s1, categorydataset, categorydataset
								.getRowKey(i), categorydataset.getColumnKey(j));
