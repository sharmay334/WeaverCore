...
public class BOLoginDaoImpl {
	...
	public LoginBean loginAuthentication(String userName, String password, String loginToken, String interfaceType,
			String ipAddress, String sessionId, short domainId, String logintry, Session session)
		...
		/*
		 * It Is Use For Add Ip For Restriction of Other IP which is not Available in
		 * List
		 */
...
//					return loginBean;
		...
		if (invalidLoginTry < maxInvalidTry || maxInvalidTry == -1) {
			userMaster.setLastLoginDate(new Timestamp(Calendar.getInstance().getTimeInMillis()));
		...
		if (checkPass) {
			...
			if (loginStatus.size() == 1) {
				...
				loginStatusBean = new StRmBoUserLoginStatus(userMaster.getDomainId(), userMaster.getUserId(), sessionId,
						ipAddress, "LOGGED_IN");
	...
	public String changePassword(UserInfoBean userBean, String oldPass, String newPass, String ipAddress,
			Short passHistory, Session session) throws NumberFormatException, PMSException {
		...
		StRmBoUserMaster userMaster = (StRmBoUserMaster) session.get(StRmBoUserMaster.class, userBean.getUserId());
		if (userMaster != null) {
			dbPass = userMaster.getPassword();
		}
	...
	public String forgotPassword(String email, Session session) {
		...
		StRmBoUserMaster userMaster = (StRmBoUserMaster) session.get(StRmBoUserMaster.class, userInfo.getUserId());
