...
public class UserMgmtDaoImpl {
	...
	public String createBoUser(UserInfoBean userInfoBean, String userName, String status, String secQues, String secAns,
			...
			String ipAddress, Session session, Short domainId, String userType) {
		status = "ACTIVE";
		...
		stBoUserMaster.setStRmBoRoleMaster(
				(StRmBoRoleMaster) session.load(StRmBoRoleMaster.class, Integer.valueOf(roleId.toString())));
		/*
		 * new StRmBoRoleMaster(Integer .valueOf(roleId.toString())));
		 */
		stBoUserMaster.setRegistrationDate(new java.sql.Timestamp(new Date().getTime()));
		stBoUserMaster.setLastLoginDate(new java.sql.Timestamp(new Date().getTime()));
		...
		criteria.setProjection(Projections.projectionList().add(Projections.property("privId"))
				.add(Projections.property("sdmId")).add(Projections.property("status")));
		...
		for (int i = 0; i < fetchPrivQueryList.size(); i++) {
			...
			stBoUserPrivMapping1.setPrivId((Integer) fetchPrivQueryList.get(i)[0]);
			stBoUserPrivMapping1.setSdmId((Short) fetchPrivQueryList.get(i)[1]);
			stBoUserPrivMapping1.setStatus(fetchPrivQueryList.get(i)[2].toString());
	...
	public boolean checkUserEmailIdAvailabilityForDomain(String emailId, Short domainId, int userId, Session session) {
		...
		criteria.setProjection(Projections.projectionList().add(Projections.property("userInfo.emailId"))
	...
	public String assignGroups(int creatorUserId, String[] rolePriv, short creatorRoleId, UserDetailsBean usrdetBean,
			int[] mappingId, int[] privCount, String ipAddress, short domainId, Session session)
		...
		stRmBoUserMaster.setIsRoleHead("Y");
		...
		stRmBoUserMaster.setRegistrationDate(new Timestamp(new Date().getTime()));
		stRmBoUserMaster.setLastLoginDate(new Timestamp(new Date().getTime()));
		stRmBoUserMaster.setStRmBoRoleMaster((StRmBoRoleMaster) session.load(StRmBoRoleMaster.class,
				Integer.valueOf(creatorRoleId)));/*
													 * new StRmBoRoleMaster(Integer .valueOf(creatorRoleId)));
													 */
		...
		userInfo.setRegion(usrdetBean.getRegion()!=null?usrdetBean.getRegion():"");
		userInfo.setBranch(usrdetBean.getBranch()!=null?usrdetBean.getBranch():"");
...
//		stRmBoUserInfo.setStRmBoUserMaster(stRmBoUserMaster);
		...
		for (StRmServiceDeliveryMaster deliveryMaster : deliveryMasterList) {
			...
			if (!("RUMMY".equalsIgnoreCase(displayName) || "IGE".equalsIgnoreCase(displayName)
					|| "SGE".equalsIgnoreCase(displayName))) {

				String privT[] = deliveryMaster.getStRmServiceMaster().getPrivRepTable().split("_");
				StringBuilder privTableName = new StringBuilder();
				privTableName.append(privT[0].substring(0, 1).toUpperCase() + privT[0].substring(1));
				for (int i = 1; i < privT.length; i++) {
					privTableName.append(privT[i].substring(0, 1).toUpperCase() + privT[i].substring(1));
				}

				Class clazz = Class.forName("com.stpl.pms.hibernate.mapping." + privTableName.toString());

				DetachedCriteria detachedCriteria = DetachedCriteria.forClass(StRmBoRolePrivMapping.class);
				detachedCriteria.add(Restrictions.eq("sdmId", deliveryMaster.getSdmId()));
				detachedCriteria.add(Restrictions.eq("roleId", Integer.valueOf(creatorRoleId)));
				detachedCriteria.setProjection(Projections.distinct(Projections.property("privId")));
				...
				criteria.setProjection(Projections.distinct(Projections.property("privId")));

				List<Integer> privIdList = criteria.list();

				criteria = session.createCriteria(clazz);
				criteria.add(Restrictions.in("privId", privIdList));
				criteria.setProjection(Projections.projectionList().add(Projections.groupProperty("privId"))
						.add(Projections.property("isDefault")));
		...
		criteria.setProjection(Projections.projectionList().add(Projections.distinct(Projections.property("sdmId")))
				.add(Projections.property("stRmServiceMaster")));
		...
		for (Object[] obj : objArr) {
			...
			if (privMap.get(Integer.parseInt(obj[0].toString())) != null) {
				...
				criteria.add(Restrictions.in("privNameCode", Arrays
						.asList(privMap.get(Integer.parseInt(obj[0].toString())).replace("'", "").split("\\s*,\\s*"))));
				criteria.add(Restrictions.isNotNull("dependentPriv"));
				criteria.setProjection(Projections.distinct(Projections.property("dependentPriv")));
				...
				dcCriteria.setProjection(Projections.distinct(Projections.property("privId")));
				dcCriteria.add(Restrictions.eq("privStatus", "ACTIVE"));

				if (depPrivs.equals("") || depPrivs.equals(null)) {
					dcCriteria.add(Restrictions.or(Restrictions.eq("isDefault", "Y"),
							Restrictions.in("privNameCode", Arrays.asList(privMap
									.get(Integer.parseInt(obj[0].toString())).replace("'", "").split("\\s*,\\s*")))));

				} else {
					dcCriteria.add(Restrictions.or(Restrictions.eq("isDefault", "Y"),
							Restrictions.or(
									Restrictions.in("privNameCode",
											Arrays.asList(privMap.get(Integer.parseInt(obj[0].toString()))
													.replace("'", "").split("\\s*,\\s*"))),
									Restrictions.in("privId", Arrays.asList(depPrivs.split("\\s*,\\s*"))))));
				}
	...
	public Map<String, TreeMap<String, TreeMap<String, TreeMap<String, List<UserPrivBean>>>>> getSubUserPriviledges(
		...
		Criteria criteria = session.createCriteria(StRmBoUserMaster.class);
		criteria.add(Restrictions.eq("userName", userName));
		criteria.setProjection(Projections.projectionList().add(Projections.property("userId"))
				.add(Projections.property("stRmBoRoleMaster.roleId")));
		...
		for (StRmServiceDeliveryMaster deliveryMaster : deliveryMasterList) {
			...
			if (!("rummy".equalsIgnoreCase(serviceName) || "IGE".equalsIgnoreCase(serviceName)
					|| "SGE".equalsIgnoreCase(serviceName))) {
				...
				criteria.setProjection(Projections.projectionList()
						.add(Projections.distinct(Projections.property("privId"))).add(Projections.property("status")));
				...
				if (objArr.size() != 0) {
					...
					criteria.add(Restrictions.eq("tier", tierType));
					criteria.setProjection(Projections.projectionList()
							.add(Projections.distinct(Projections.property("privNameCode")))
							.add(Projections.groupProperty("relatedTo")).add(Projections.groupProperty("groupNameCode"))
							.add(Projections.property("isDefaultGroup")).add(Projections.groupProperty("privId"))
							.add(Projections.property("dependentPriv")));
					...
					if (!headPriviledgeMap.containsKey(deliveryMaster.getServiceDisplayName())) {
						interfaceMap = new TreeMap<String, TreeMap<String, TreeMap<String, List<UserPrivBean>>>>();
						headPriviledgeMap.put(deliveryMaster.getServiceDisplayName(), interfaceMap);
					}

					TreeMap<String, List<UserPrivBean>> groupNameMap = null;
					interfaceMap = headPriviledgeMap.get(deliveryMaster.getServiceDisplayName());
					...
					if (privMap.size() > 0) {
						interfaceMap.put(deliveryMaster.getServiceDisplayName() + "-" + deliveryMaster.getSdmId(),
								privMap);
					}
					if (interfaceMap.size() == 0) {
						headPriviledgeMap.remove(deliveryMaster.getServiceDisplayName());
					}
				}
			}
		}
	...
	public void editSubUserPriviledges(String userName, String[] rolePriv, int[] mappingId, int[] privCount,
		...
		criteria = session.createCriteria(StRmServiceDeliveryMaster.class).createAlias("stRmServiceMaster",
				"serviceMaster");
		criteria.add(Restrictions.in("sdmId", strMappingIds));
		criteria.setProjection(Projections.projectionList().add(Projections.property("sdmId"))
				.add(Projections.property("serviceMaster.privRepTable")));
		...
		for (Object[] obj : fetchPrivTableList) {
			if (privMap.get(Integer.valueOf(obj[0].toString())) != null) {
				...
				criteria = session.createCriteria(clazz);
				criteria.add(Restrictions.eq("privStatus", "ACTIVE"));
				criteria.add(Restrictions.in("privNameCode", Arrays.asList(privMap
						.get(Integer.valueOf(obj[0].toString())).toString().replace("'", "").split("\\s*,\\s*"))));
				criteria.add(Restrictions.isNotNull("dependentPriv"));
				criteria.setProjection(Projections.distinct(Projections.property("dependentPriv")));
				List<String> dependentPrivList = criteria.list();
				...
				if (depPrivs != null) {
					...
					dcCriteria.add(Restrictions.or(Restrictions.eq("isDefault", "Y"),
							Restrictions.or(
									Restrictions.in("privNameCode",
											Arrays.asList(privMap.get(Integer.valueOf(obj[0].toString()))
													.replace("'", "").split("\\s*,\\s*"))),
									Restrictions.in("privId", Arrays.asList(depPrivs.split("\\s*,\\s*"))))));
				} else {
					dcCriteria.add(Restrictions.or(Restrictions.eq("isDefault", "Y"),
							Restrictions.in("privNameCode", Arrays.asList(privMap
									.get(Integer.valueOf(obj[0].toString())).replace("'", "").split("\\s*,\\s*")))));
				}
	...
	public List<UserDetailsBean> searchBoUsers(String userName, int roleId, String type, String status,
			UserInfoBean parentUserBean, Session session) {
...
//						+ nameArr[0].trim() + "'%"));
				for (int i = 1; i < nameArr.length; i++) {
					if (!"".equals(nameArr[i].trim())) {
						str = nameArr[1].trim() + "%";
						detachedCriteria.add(Restrictions.like("lastName", str));
					}
				}
			}
		}
	...
	public UserDetailsBean showBOUserDetails(int userId, String type, Session session) {
		UserDetailsBean userBean = null;

		StRmBoUserMaster stRmBoUserMaster = (StRmBoUserMaster) session.load(StRmBoUserMaster.class, userId);
		...
		String userDepartment = stRmBoUserMaster.getStRmBoRoleMaster().getRoleName();
		userBean.setDepartment(userDepartment);

		return userBean;
	}

	public String resetPassBO(int userId, String autoPass, String emailId, String userName, String firstName,
			String lastName, Session session) {
	...
	public Map<String, TreeMap<String, TreeMap<String, TreeMap<String, List<UserPrivBean>>>>> getDomainPriviledges(
			String tierType, Session session) throws ClassNotFoundException {

		Integer domainId = 0;
		Map<String, TreeMap<String, TreeMap<String, TreeMap<String, List<UserPrivBean>>>>> headPriviledgeMap = new TreeMap<String, TreeMap<String, TreeMap<String, TreeMap<String, List<UserPrivBean>>>>>();
		Criteria criteria = session.createCriteria(StRmServiceDeliveryMaster.class).setCacheable(true);
	...
	public String createDomainUser(UserInfoBean userInfoBean, String userName, String status, String secQues,
			String secAns, String firstName, String lastName, String gender, String email, String phone,
			String roleIdStr, String ipAddress, Session session, Short domainId, String userType, String[] rolePriv,
			int[] mappingId, int[] privCount, String[] rolePrivRUMMY) {
		status = "ACTIVE";
		Criteria criteria = session.createCriteria(StRmBoUserMaster.class);
		criteria.add(Restrictions.eq("userName", userName));
		criteria.setProjection(Projections.property("userName"));
		...
		//
		saveDomainUserPriviledge(rolePriv, privCount, userType, newUserId, "" + roleIdStr, domainId, rolePrivRUMMY,
				session);
		//
	...
	public void saveDomainUserPriviledge(String[] rolePriv, int[] privCount, String tier, Integer userId, String roleId,
			Short domainId, String[] rolePrivRUMMY, Session session) {
		...
		try {
			...
			for (Object[] obj : objArr) {
				...
				if (privMap.get(Integer.parseInt(obj[0].toString())) != null) {
					...
					criteria = session.createCriteria(clazz);
					criteria.add(Restrictions.eq("privStatus", "ACTIVE"));
					criteria.add(Restrictions.in("privNameCode", Arrays.asList(
					...
					if (depPrivs.equals("") || depPrivs.equals(null)) {
						dcCriteria.add(Restrictions.or(Restrictions.eq("isDefault", "Y"),
								Restrictions.in("privNameCode",
										Arrays.asList(privMap.get(Integer.parseInt(obj[0].toString())).replace("'", "")
												.split("\\s*,\\s*")))));
	...
	public Map<String, TreeMap<String, TreeMap<String, TreeMap<String, List<UserPrivBean>>>>> getDomainUserPriviledges(
			String userName, String tierType, Session session) throws ClassNotFoundException {
		...
		criteria.add(Restrictions.eq("userName", userName));
		criteria.setProjection(Projections.projectionList().add(Projections.property("userId"))
				.add(Projections.property("stRmBoRoleMaster.roleId")).add(Projections.property("domainId")));
	...
	public void editDomainUserPriviledges(String userName, String[] rolePriv, int[] mappingId, int[] privCount,
			String rolePrivRUMMY[], Session session) throws ClassNotFoundException {
		int userId = 0, roleId = 0;
		Short domainId = 0;
	...
	public UserInfoBean getInpersonatedBean(Integer boUserId, Session session) throws PMSException {
	...
	public Map<Integer, String> getBoUserMap(Integer selfId, Session session) {
		...
		List<Object[]> boUsrMasterList = session.createCriteria(StRmBoUserMaster.class)
				.add(Restrictions.ne("userId", selfId)).add(Restrictions.eq("status", "ACTIVE"))
	...
	public String assignGroups(int creatorUserId, String[] rolePriv, short creatorRoleId, EmployeeBean usrdetBean,
			int[] mappingId, int[] privCount, String ipAddress, short domainId, Session session)
			throws ClassNotFoundException {
		...
		Criteria criteria1 = session.createCriteria(StRmBoUserMaster.class);
		criteria1.add(Restrictions.eq("userName", usrdetBean.getEmpUserName()));
		criteria1.setProjection(Projections.property("userName"));
		List<String> result = criteria1.list();
		...
		stRmBoUserMaster.setStRmBoRoleMaster((StRmBoRoleMaster) session.load(StRmBoRoleMaster.class,
				Integer.valueOf(creatorRoleId)));/*
													 ...
													 */
		stRmBoUserMaster.setStatus(usrdetBean.getStatus());
		stRmBoUserMaster.setUserName(usrdetBean.getEmpUserName());
		stRmBoUserMaster.setUserAccessType("ADMIN");
		stRmBoUserMaster.setInvalidLoginTry((short) 0);
		StRmBoUserInfo userInfo = new StRmBoUserInfo();
		userInfo.setEmailId(usrdetBean.getEmailId());
		userInfo.setFirstName(usrdetBean.getEmpName());
		userInfo.setGender(usrdetBean.getGender());
		// userInfo.setCity(usrdetBean.get)
		userInfo.setPhoneNum(usrdetBean.getPhoneNbr());
		userInfo.setAddressLine1("address");
